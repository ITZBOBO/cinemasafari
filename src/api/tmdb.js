...
const API_KEY = import.meta.env.VITE_TMDB_API_KEY || "e3abdf93016ffa6835e315ecc4da4436";
...
export async function discoverMoviesByGenre(genreId, page = 1) {
  const url = `${BASE_URL}/discover/movie?api_key=${API_KEY}&with_genres=${genreId}&page=${page}&include_adult=false`;
  const res = await fetch(url, { headers });
